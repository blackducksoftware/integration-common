...
public final class NullSafe<T> {
     ...
     */
    public <U> NullSafe<U> then(Function<T, U> methodRef) {
    ...
    /**
     * Useful when not chaining calls with the then method. Let's say that
     ...
     * <pre>
     *     {@code
     *     NullSafe.of(user).then(User::getEmail).get();
     *     }
     * </pre>
     *
     ...
     *
     * @param exception an exception that is thrown if the wrapped value is null
     * @return the value wrapped by this instance of NullSafe if it is not null
     * @throws Throwable when the wrapped value is null
     */
    public T getOrThrow(Exception exception) throws Exception {
        if(t == null) {
            throw exception;
        } else {
            return t;
